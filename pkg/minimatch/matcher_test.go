...
var testItems = []testStruct{
	{
		pattern: "a*",
		expect:  []string{"a", "abc", "abd", "abe"},
	},
	{
		pattern: "X*",
		expect:  []string{"X*"},
		options: minimatch.Options{NoNull: true},
	},
	{
		pattern: "X*",
		expect:  []string{},
	...
	{
		pattern: "[a-c]b*",
		expect:  []string{"abc", "abd", "abe", "bb", "cb"},
	},
...
func TestMatchBasic(t *testing.T) {
	for _, item := range testItems {
		var matches []string
		if item.files != nil {
			matches = minimatch.Match(item.files, item.pattern, item.options)
		} else {
			matches = minimatch.Match(files, item.pattern, item.options)
		}

		assert.ElementsMatch(t, matches, item.expect, item.pattern)
	}
}
